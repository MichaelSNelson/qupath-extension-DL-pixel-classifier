...
public class LayerFreezePanel extends VBox {
    ...
    public LayerFreezePanel() {
        ...
        Label infoLabel = new Label("The model starts with features learned from millions of images. "
        ...
        learnMoreLink.setOnAction(e -> {
            try {
                java.awt.Desktop.getDesktop()
                        .browse(java.net.URI.create("https://course.fast.ai/Lessons/lesson6.html"));
            } catch (Exception ex) {
        ...
        presetLabel.setTooltip(TooltipHelper.create("How much of the pretrained model to keep vs. retrain.\n"
        ...
        contextWarningLabel.setStyle("-fx-background-color: #FFF3CD; -fx-border-color: #FFECB5; "
     ...
     */
    public void loadLayers(String architecture, String encoder, int numChannels, int numClasses, int contextScale) {
        ...
        if (backend != null) {
            try {
                List<ClassifierClient.LayerInfo> layerInfos =
                        backend.getModelLayers(architecture, encoder, numChannels, numClasses);
                ...
                if (layerInfos != null && !layerInfos.isEmpty()) {
                    ...
                    logger.info("Loaded {} layers from server for {}/{}", layerInfos.size(), architecture, encoder);
                    return;
                }
            } catch (Exception e) {
                logger.debug("Server layer loading failed for {}/{}: {}", architecture, encoder, e.getMessage());
            }
        }
    ...
    private List<LayerItem> buildLocalLayerStructure(String encoder) {
        ...
        if ("resnet".equals(family)) {
            ...
            boolean isResnet50Plus = encoder != null && (encoder.contains("50") || encoder.contains("101"));
            result.add(makeLayer(
                    "encoder.conv1",
                    "Encoder: Initial Conv",
                    0,
                    true,
                    isResnet50Plus ? 9_408 : 9_408,
                    true,
                    isHistology));
            result.add(makeLayer(
                    "encoder.layer1",
            ...
            result.add(makeLayer(
                    ...
                    2,
                    true,
                    isResnet50Plus ? 1_219_584 : 295_424,
                    true,
                    isHistology));
            result.add(makeLayer(
                    "encoder.layer3",
                    ...
                    3,
                    true,
                    isResnet50Plus ? 7_098_368 : 1_180_672,
                    false,
                    isHistology));
            result.add(makeLayer(
                    "encoder.layer4",
                    "Encoder: Block 4 (512 filters)",
                    4,
                    true,
                    isResnet50Plus ? 14_964_736 : 4_720_640,
                    false,
                    isHistology));
        } else if ("efficientnet".equals(family)) {
            result.add(makeLayer("encoder._conv_stem", "Encoder: Stem Conv", 0, true, 864, true, false));
            result.add(makeLayer("encoder._blocks[0:4]", "Encoder: Blocks 0-3", 1, true, 15_000, true, false));
            result.add(makeLayer("encoder._blocks[4:10]", "Encoder: Blocks 4-9", 2, true, 60_000, true, false));
            result.add(makeLayer("encoder._blocks[10:18]", "Encoder: Blocks 10-17", 3, true, 300_000, false, false));
            result.add(makeLayer("encoder._blocks[18:]", "Encoder: Blocks 18+", 4, true, 500_000, false, false));
        } else if ("densenet".equals(family)) {
            result.add(makeLayer("encoder.features.conv0", "Encoder: Initial Conv", 0, true, 9_408, true, false));
            result.add(
                    makeLayer("encoder.features.denseblock1", "Encoder: Dense Block 1", 1, true, 340_000, true, false));
            result.add(
                    makeLayer("encoder.features.denseblock2", "Encoder: Dense Block 2", 2, true, 920_000, true, false));
            result.add(makeLayer(
                    "encoder.features.denseblock3", "Encoder: Dense Block 3", 3, true, 2_800_000, false, false));
            result.add(makeLayer(
                    "encoder.features.denseblock4", "Encoder: Dense Block 4", 4, true, 2_000_000, false, false));
        } else if ("vgg".equals(family)) {
            result.add(makeLayer(
                    "encoder.features[0:7]", "Encoder: Layers 1-2 (64 filters)", 0, true, 38_720, true, false));
            result.add(makeLayer(
                    "encoder.features[7:14]", "Encoder: Layers 3-4 (128 filters)", 1, true, 221_440, true, false));
            result.add(makeLayer(
                    "encoder.features[14:24]", "Encoder: Layers 5-7 (256 filters)", 2, true, 1_475_328, true, false));
            result.add(makeLayer(
                    "encoder.features[24:34]", "Encoder: Layers 8-10 (512 filters)", 3, true, 5_899_776, false, false));
            result.add(makeLayer(
                    "encoder.features[34:]", "Encoder: Layers 11-13 (512 filters)", 4, true, 5_899_776, false, false));
        } else if ("mobilenet".equals(family)) {
            result.add(makeLayer("encoder.features[0:2]", "Encoder: Initial Conv", 0, true, 1_200, true, false));
            result.add(makeLayer("encoder.features[2:5]", "Encoder: Blocks 1-3", 1, true, 15_000, true, false));
            result.add(makeLayer("encoder.features[5:9]", "Encoder: Blocks 4-7", 2, true, 100_000, true, false));
            result.add(makeLayer("encoder.features[9:14]", "Encoder: Blocks 8-12", 3, true, 500_000, false, false));
            result.add(makeLayer("encoder.features[14:]", "Encoder: Blocks 13+", 4, true, 1_200_000, false, false));
        } else if ("se_resnet".equals(family)) {
            result.add(makeLayer("encoder.layer0", "Encoder: Initial Conv", 0, true, 9_408, true, false));
            result.add(makeLayer("encoder.layer1", "Encoder: SE Block 1 (64 filters)", 1, true, 300_000, true, false));
            result.add(
                    makeLayer("encoder.layer2", "Encoder: SE Block 2 (128 filters)", 2, true, 1_500_000, true, false));
            result.add(
                    makeLayer("encoder.layer3", "Encoder: SE Block 3 (256 filters)", 3, true, 8_500_000, false, false));
            result.add(makeLayer(
                    "encoder.layer4", "Encoder: SE Block 4 (512 filters)", 4, true, 17_000_000, false, false));
        } else {
            // Generic fallback for unknown encoders
            result.add(makeLayer("encoder.layer_early", "Encoder: Early Layers", 0, true, 0, true, false));
            result.add(makeLayer("encoder.layer_mid", "Encoder: Middle Layers", 2, true, 0, false, false));
            result.add(makeLayer("encoder.layer_late", "Encoder: Late Layers", 4, true, 0, false, false));
        }

        // Add decoder and segmentation head (always present)
        result.add(new LayerItem(
                "decoder",
                "Decoder (all layers)",
                0,
                false,
    ...
    private LayerItem makeLayer(
        ...
        LayerItem item = new LayerItem(name, displayName, paramCount, isEncoder, depth, recommendedFreeze, description);
    ...
    private void applyPreset() {
        ...
        if (backend != null) {
            try {
                recommendations = backend.getFreezeRecommendations(datasetSize, currentEncoder);
            } catch (Exception e) {
        ...
        updateStatus();
        logger.info(
                "Applied {} preset for encoder {}: {} layers frozen",
                datasetSize,
                currentEncoder,
                getFrozenLayerNames().size());
    }
     ...
     */
    private Map<Integer, Boolean> getLocalFreezeRecommendations(String datasetSize, String encoder) {
        boolean isHistology = encoder != null
                && (encoder.contains("lunit") || encoder.contains("kather") || encoder.contains("tcga"));
    ...
    private void updateContextWarning() {
        logger.info("updateContextWarning called: contextScale={}, layerCount={}", currentContextScale, layers.size());
        ...
        boolean firstLayerFrozen = layers.stream().anyMatch(l -> l.getDepth() == 0 && l.isEncoder() && l.isFrozen());
        logger.info("Context warning: firstLayerFrozen={}", firstLayerFrozen);

        if (firstLayerFrozen) {
            contextWarningLabel.setText("Warning: Context scale is active (" + currentContextScale
            ...
            contextWarningLabel.setText("Note: Context scale is active (" + currentContextScale
                    + "x). The initial conv layer is correctly set to trainable so "
                    + "it can learn to use the extra context channels.");
            contextWarningLabel.setStyle("-fx-background-color: #D4EDDA; -fx-border-color: #C3E6CB; "
                    + "-fx-border-radius: 4; -fx-background-radius: 4; -fx-padding: 8;");
        }
    ...
    public static class LayerItem {
        ...
        public boolean isRecommendedFreeze() {
            return recommendedFreeze;
        }
